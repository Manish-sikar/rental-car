      ...
      if (newRecord) {
        newRecord.addEventListener("click", function () {
          ...
          (offCanvasElement.querySelector(".dt-course-name").value = ""),
            $(".dt-select-status").val(0).trigger("change");
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: customPath + "json/courses.json",
      columns: [
        { data: "" },
        { data: "id" },
        { data: "id" },
    ...
    $("div.head-label").html('<h5 class="card-title mb-0">Course List</h5>');
  }
  ...
  fv.on("core.form.valid", function () {
    var $course_name = $(".add-new-record .dt-course-name").val(),
      $status = $(".add-new-record .dt-select-status").val();
